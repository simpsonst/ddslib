...
/* PFX: prefix; vstr, vwcs
   ...
   NAIVE: If 2, disable the set/append/insertf functions.  If
   non-zero, use PRINT with increasing allocations until it fits.
   Otherwise, use the function once with a NULL buffer to get the
   required size, allocate that amount, and use the function again
   with the new space. */
