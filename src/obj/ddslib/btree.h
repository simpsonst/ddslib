...
#define btree_DECL(P, T, KT)                            \
  ...
  P##_elem *P##_find(T *par, T **rootp, KT key,         \
                     P##_elem *res, void *cmpctxt);     \
...
#define btree_checkchild(M, OP, D)                                      \
  ((OP)->M.child[D] != 0 &&                                             \
   (assert((OP)->M.child[D]->M.holder == &(OP)->M.child[D]), 1))

#define btree_checknode(M, OP)                                          \
  (btree_checkchild(M, (OP), btree_LEFT),                               \
...
#define btree_IMPL(P, T, KT, M, K, CMP)                                 \
  ...
  {                                                                     \
    ...
    if (!tmp.child[0]) {                                                \
      ...
      /* Make the left child replace this node, and find a new place    \
         for the right child if it exists. */                           \
